...
def get_model(trace_length, units, optimizer):
    ...
    Local = LocallyConnected1D(filters=1, kernel_size=108, strides=54, padding='valid', activation=None, use_bias=True
...
def get_ciphertext_matrix(test_index, trs_file_path):
    trs_file_path = trs_file_path
    length = len(test_index)
    cipher_text_need = np.zeros((length,16))
    
    count = 0
    for i in test_index:
        trs = trs_file_path + '{:0>6d}.npz'.format(i)
        trace_struc = np.load(trs)       
        cipher = trace_struc["cipher"]
        cipher_text_need[count,:] = cipher
        
        count += 1
    return cipher_text_need
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes_for_batch):
        ...
        for i in indexes_for_batch:
            ...
            cipher = trace_struc["cipher"]
            label_value = inv_sbox[key_suppose^int(cipher[11])]^int(cipher[7])
    ...
    def on_epoch_end(self, epoch, logs=None):
        if((epoch+1)%epochs_per_save==0):
            ...
            if((epoch+epoch_offset+1)%20==0):
                temp_model.save('./models/HD_test/Test_6_2_fulltrace_cipher_byte_7_epoch{:0>6d}.hdf5'.format(epoch+epoch_offset+1))
        

            predict_out = temp_model.predict_generator(DataGenerator(index_in=test_index[i, :], **params_predict),
                                           steps=test_num//batch_size, max_queue_size=10)
            distance_out_array = predict_out.argmax(axis=-1)
            key_list = []
            cipher_matrix = get_ciphertext_matrix(test_index[i, :], trs_file_path)
        
            for m in range(len(distance_out_array)):
                if m < 100:
                    attack_step = attack_step_s
                else: 
                    attack_step = attack_step_l
                    
                k = Sbox[distance_out_array[m]^int(cipher_matrix[m,7])] ^ int(cipher_matrix[m,11])
            ...
            for j in range(len(distance_out_array)):
                ...
                for key in range(256):
                    inter_value = inv_sbox[key^int(cipher_matrix[j,11])]^int(cipher_matrix[j,7])
                    inter_value_pro[key] += np.log(predict_out[j][inter_value])
...
test_index = np.expand_dims(test_index, axis = 0)
...
trs_file_path = '/home/data/disk_nvme2/AES_HD/split/trace'
...
checkpointer_acc = ModelCheckpoint(filepath='./models/HD_test/Test_6_2_fulltrace_cipher_byte_7_best_acc.hdf5'.format(batch_size), monitor='val_acc', verbose=0, save_best_only=True, save_weights_only=False, mode='max', period=1)
checkpointer_loss = ModelCheckpoint(filepath='./models/HD_test/Test_6_2_fulltrace_cipher_byte_7_best_loss.hdf5'.format(batch_size), monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)
csv_logger = keras.callbacks.CSVLogger(filename='./models/HD_test/tensor_log/Test_6_2_fulltrace_cipher_byte_7_log1.csv', append=True, separator=' ')
...
model.fit_generator(DataGenerator(index_in=train_index, **params_train),
                    ...
                    validation_data = DataGenerator(index_in=test_index[0, :], **params_valid),
                    validation_steps = len(test_index[0, :])//batch_size, 
                    callbacks = [mycallback, lrate, checkpointer_acc, checkpointer_loss, csv_logger, terminate_on_nan],
