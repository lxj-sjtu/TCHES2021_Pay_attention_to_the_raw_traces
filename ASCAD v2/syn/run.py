...
def get_model(trace_length, units, optimizer):
    ...
    input_pad = ZeroPadding1D((0, 62))(_input)
    input_cut = Lambda(lambda x:x[:,62:,:])(input_pad)

    Local1 = LocallyConnected1D(filters=1, kernel_size=125, strides=125, padding='valid', activation=None, use_bias=True,
                               kernel_regularizer=regularizers.l2(1e-3), 
                               bias_regularizer=regularizers.l2(1e-3)
                              )(_input)
    Local_overlap = LocallyConnected1D(filters=1, kernel_size=125, strides=125, padding='valid', activation=None, use_bias=True,
                               kernel_regularizer=regularizers.l2(1e-3), 
                               bias_regularizer=regularizers.l2(1e-3)
                              )(input_cut)

    Local = Concatenate()([Local1, Local_overlap])
...
def plot_weights_of_layer1_and_att(temp_model, trs):
    ...
    trace_att_sum = np.zeros(((trace_length//125),))
    trace_att_sum2 = np.zeros(((trace_length//125),))
    
    for index in range(290000,290100):
...
def calc_GE(inter_value_pro, key_target):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if((epoch+1)%epochs_per_save==0):
            ...
            for j in range(len(sout_array)):
                ...
                if j<pic_num:
                    entropy, sorted_pro, sorted_index = calc_GE(inter_value_pro, key_target)
                    pic_GE[j] = entropy
                    if j%attack_step == 0:
...
key_target = key_array[byte_index]
...
model.fit_generator(DataGenerator(index_in=train_index, **params_train),
                    ...
                    callbacks = [mycallback, lrate, checkpointer_acc, checkpointer_loss, csv_logger], use_multiprocessing=True, workers = 2,verbose=1)
