...
def get_model(trace_length, units, optimizer):
    ...
    input_pad1 = ZeroPadding1D((252, 0))(_input)
    # first kernel size = half clock
    Conv1 = Conv1D(filters=4, kernel_size=63, strides=1, padding='valid', activation=None, use_bias=True
...
#             trace = trace/self.trace_std
        ...
        if index==50000:
            # ouput only one time since same when training mode
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes_for_batch):
        ...
        for i in indexes_for_batch:
            ...
            if self.shuffle:
                augment_shift = np.random.randint(-self.augment_shift_scale,self.augment_shift_scale)
            else:
                augment_shift = 0
            ...
            with np.load(trs) as trace_struc:
                trace = trace_struc["trace"]
                label = trace_struc['label_sout']
        
#             trace = trace[self.trace_offset:self.trace_offset+self.trace_length]
            trace = trace[trace_offset+rand_shift+augment_shift:trace_offset+rand_shift+trace_length+augment_shift]
            trace = np.reshape(trace,(-1, 1))
#             #normalization 
#             trace = trace - self.trace_mean
#             trace = trace/self.trace_std
...
byte_index = 2
key_array = [0, 17, 34, 51, 68, 85, 102, 119, 136, 153, 170, 187, 204, 221, 238, 255]
...
train_index = np.asarray(train_index_list)
test_index = np.asarray(test_index_list)
train_num = 200000
test_num = 10000

train_index = train_index[:train_num]
test_index = test_index[:test_num]
...
lab_file_path = ''
trs_file_path = '/home/data/disk_nvme/ASCAD_v2/np_trace/trace'
...
fixed_shift_array = np.random.randint(-shift_scale,shift_scale,size=(300000,))
...
shift_save_file = './models/shift_numpy_array/Test_1_shift_array.npy'
if os.path.isfile(shift_save_file):
    print('but shift file EXIST, loading...')
    fixed_shift_array = np.load(shift_save_file)
    print('loaded')
    print('loaded fixed_shift_array', fixed_shift_array)
    minval = fixed_shift_array[0]
    maxval = fixed_shift_array[0]
    for i in fixed_shift_array:
        if i < minval:
