...
class SaveModelCllaBack(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if((epoch+1)%epochs_per_save==0):
            ...
            temp_model.save('./models/Test_1_byte2_7_long_leak_interval_newest.hdf5')
            if((epoch+epoch_offset+1)%25==0):
                temp_model.save('./models/Test_1_byte2_7_long_leak_interval_epoch{:0>6d}.hdf5'.format(epoch+epoch_offset+1))    
...
shift_save_file = './models/shift_numpy_array/Test_1_byte2_7_long_leak_interval_shift_array.npy' 
...
checkpointer_acc = ModelCheckpoint(filepath='./models/Test_1_byte2_7_long_leak_interval_best_acc.hdf5'.format(batch_size), monitor='val_acc', verbose=0, save_best_only=True, save_weights_only=False, mode='max', period=1)
checkpointer_loss = ModelCheckpoint(filepath='./models/Test_1_byte2_7_long_leak_interval_best_loss.hdf5'.format(batch_size), monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)
csv_logger = keras.callbacks.CSVLogger(filename='./models/tensor_log/Test_1_byte2_7_long_leak_interval_log1.csv', append=True, separator=' ')
