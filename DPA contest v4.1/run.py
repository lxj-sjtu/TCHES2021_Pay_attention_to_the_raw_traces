...
from keras.layers import Conv1D, Conv2D, AveragePooling1D, MaxPooling1D, MaxPooling2D, Concatenate
...
def get_model(trace_length, units, optimizer):
    ...
    Local = LocallyConnected1D(filters=1, kernel_size=70, strides=35, padding='valid', activation=None, use_bias=True,
...
#                           [temp_model.layers[outputlayer2].output])
    ...
    trace_att_sum = np.zeros(((trace_length//35-1)//2,))
#     trace_att_sum2 = np.zeros(((trace_length//35-1)//2,))
    
    for index in range(39000,39100):
#         print(index)
...
def get_plaintext(test_index, lab_file_path, byte_index):
    plain_text_path =  lab_file_path
    ...
    for i in test_index:
        ...
        label_struct = np.load(position)
        plain_text_need[count] = label_struct['plain_text'][byte_index]
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes_for_batch):
        ...
        for i in indexes_for_batch:
            ...
            label_value = label_pair["label"][self.byte_index]
...
class SaveModelCllaBack(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if((epoch+1)%epochs_per_save==0):
            ...
            if((epoch+1)%20==0):
                temp_model.save('./models/Test_1_epoch{:0>6d}.hdf5'.format(epoch+1))

            score = temp_model.evaluate_generator(DataGenerator(index_in=test_index, **params_valid),
            ...
            key_list = []
            p_array = get_plaintext(test_index, lab_file_path, byte_index)
            ...
            for j in range(len(sout_array)):
                ...
                if j%attack_step == 0:
                    ...
                    posi_of_key = np.where(sorted_index==key_target)[0]
            ...
            plot_weights_of_layer1_and_att(temp_model, trs_file_path, 12,12)
...
byte_index = 0
lab_file_path = '/home/data/disk_nvme/dpav41/DPA_contestv4_rsm/label/DPA_v41_label'
trs_file_path = '/home/data/disk_nvme/dpav41/DPA_contestv4_rsm/total_trace/dpav4_trace'

full_key = [108,236,198,127,40,125,8,61,235,135,102,240,115,139,54,207]
key_target = full_key[byte_index]

train_index = np.arange(39000)
test_index = np.arange(39000,40000)
train_num = 39000
test_num = 1000

attack_step_large = 100
