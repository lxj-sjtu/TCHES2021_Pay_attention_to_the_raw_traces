...
def plot_weights_of_layer1_and_att(temp_model, trs):
    ...
    trace_att_sum = np.zeros(((trace_length//22-1)//2,))
...
class SaveModelCllaBack(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if((epoch+1)%epochs_per_save==0):
            ...
            temp_model.save('./models/Test_1_byte4_6_newest.hdf5'.format(batch_size))
            if((epoch+1)%50==0):
                temp_model.save('./models/Test_1_byte4_6_epoch{:0>6d}.hdf5'.format(epoch+1))    
...
checkpointer_acc = ModelCheckpoint(filepath='./models/Test_1_byte4_6_best_acc.hdf5'.format(batch_size), monitor='val_acc', verbose=0, save_best_only=True, save_weights_only=False, mode='max', period=1)
checkpointer_loss = ModelCheckpoint(filepath='./models/Test_1_byte4_6_best_loss.hdf5'.format(batch_size), monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)
csv_logger = keras.callbacks.CSVLogger(filename='./models/tensor_log/Test_1_byte4_6_log1.csv', append=True, separator=' ')
